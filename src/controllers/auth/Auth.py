...
def post_register(data):
    ...
    if existing_email:
        return jsonify({"success": False, "status code": 409, "message": "User already exists"}), 409
    ...
    db.session.commit()
    return jsonify({"success": True, "status code": 201, "message": "User registered successfully"}), 201
